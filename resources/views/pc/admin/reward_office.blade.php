	...
	<div class="block">
		<div class="right paddingtop30">
			<a href="#!/reward/setting/view" class="periodselected period">
				<i class="glyphicon glyphicon-heart text-white"></i> @lang('lang.rew_red_rule_setting')
			</a>
		</div>
		...
		<table class="table">
			...
			<tbody>
				<tr ng-repeat="item in list_data.data">
					...
					<td>
						{? item.user.name ?}
						<span ng-if="item.is_approval==0&&item.is_proposal==1" class="text-danger text-bold">@lang('lang.aleady_apply')</span>
					</td>
					...
					<td ng-if="search.type==0">
						<a class="stockservicebtn stockservicedownload bgyellow" ng-click="onGive(item.id, item.dealer.name, item.user.name, item.red_packet_setting.redpacket_price)">
							@lang('lang.rew_red_envelopes')
						</a>
					</td>
