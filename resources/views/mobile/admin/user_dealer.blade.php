<div class="user_v_content page_wrapper" style="display: none;">
	...
	<div class="info_panel bg_white" style="font-size: 12px;">
		<div class="item_my" style="padding: 10px 10px 5px 10px;">
			<div class="row info_header margin0" style="background-color: #eaffc5;">
				...
				<div class="col-xs-3 text-right nopadding">
					<a class="col-xs-12 btn btn-primary" style="font-size: 12px;" ng-href="#!/user/dealer/edit/modify/{? (dealer.detail_id == null ? 'dealer' : 'store' )?}/{?dealer.id?}">
						@lang('lang.pr_item_edit')</a>
				</div>
			</div>
		</div>

		<div class="bg_white item row" style="padding: 5px 10px; margin: 0px 10px;" ng-show="dealer.corp_info.name != null">
			<div class="col-xs-5 text-right">@lang('lang.corporation')</div>
			<div class="col-xs-6 col-xs-offset-1">{? dealer.corp_info.name ?}</div>
		</div>
		<div class="bg_white item row" style="padding: 5px 10px; margin: 0px 10px;" ng-show="dealer.level != 0">
		...
		<div class="bg_white item row" style="padding: 5px 10px; margin: 0px 10px;">
	...
	<div class="info_panel bg_white" style="font-size: 12px;">
		...
		<div class="bg_white item out_border1 row" ng-repeat="user in users" style="padding: 5px 10px; margin: 0px 10px;">
			<div class="col-xs-4 nopadding text-center">{? user.name ?}</div>
			<div class="col-xs-2 nopadding text-center">{? user.role.name ?}</div>
			<div class="col-xs-4 nopadding text-center"><i class="glyphicon glyphicon-phone" aria-hidden="true"></i>{?user.link?}</div>
			<div class="col-xs-2 nopadding text-center"><a href="#!/user/staff/detail/{?user.id?}" class="user_view_btn">@lang('lang.ov_details')</a></div>
		</div>	
	</div>
		...
		<div class="bg_white item out_border1" ng-repeat="lower_dealer in lower_dealers" style="padding: 5px 10px; margin:5px 10px;">
			...
			<div class="col-xs-6 nopadding">
				<div class="col-xs-12 nopadding">
					<div class="col-xs-6 nopadding text-right">@lang('lang.label_stock')</div>
					<div class="col-xs-6 nopadding text-center">{? lower_dealer.total_stock ?}</div>
				</div>
				<div class="col-xs-12 nopadding">
					<div class="col-xs-6 nopadding text-right">@lang('lang.sl_total_sale_amount')</div>
					<div class="col-xs-6 nopadding text-center">{? lower_dealer.total_sale ?}</div>
				</div>
				<div class="col-xs-12 nopadding">
					<div class="col-xs-6 nopadding text-right">@lang('lang.u_monthly_settlement_amount')</div>
					<div class="col-xs-6 nopadding text-center">{? lower_dealer.total_unbalance | floor ?}</div>
				</div>
				<div class="col-xs-12 nopadding">
					<div class="col-xs-6 nopadding text-right">@lang('lang.u_monthly_settlement_promotion')</div>
					<div class="col-xs-6 nopadding text-center">{? lower_dealer.total_promotion | floor ?}</div>
				</div>
			</div>
		...
		<div class="modal-dialog width90">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<p class="text-center fontsize1_2 txt_bold">@lang('successful_setting_balance')</p>
				</div>
				<div class="modal-footer">
					<div class="col-xs-12 nopadding text-center">
						<button type="button" class="btn bgf90 color_white">@lang('lang.confirm')</button>
					</div>
				</div>
			</div>
		</div>
